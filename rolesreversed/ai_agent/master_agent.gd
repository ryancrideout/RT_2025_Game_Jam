...
func spawn_building(building_scene: PackedScene, building_name, building_position) -> bool:
    ...
    var building_size = self.get_node("Buildings").get_children().size()

    if building_size > 10:
        #print("ERROR: Maximum number of buildings reached!")
        return false
...
func spawn_free_unit():
    ...
    if buildings.size() > 0:
        ...
        if is_instance_valid(building):
            if building.has_method("spawn_unit"):
                building.spawn_basic_free_unit()
                unit_spawned = true
            else:
                print("WARNING: Building does not have spawn_unit method")
