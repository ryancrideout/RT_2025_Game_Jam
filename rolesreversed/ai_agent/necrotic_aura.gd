...
@export var resource_cost_per_hit: int = 2
...
func _on_resource_changed(resource_type, new_value):
	if resource_type == "secondary":
		var ratio :float = clamp(float(new_value) / 10000, 0.0, 1.0)
...
func _on_damage_timer_timeout():
	...
	for body in self.get_overlapping_bodies():
		if body is Skeleton:
			...
			agent_owner_resources.update_primary_resource(resource_cost_per_hit / 2)
		elif body is Human:
			body.receive_damage(incoming_damage)
			agent_owner_resources.update_primary_resource(resource_cost_per_hit)
